...
- **мобильный MVP** (`mobile/flutter_taxi_app`, пакеты `shared_core`, `passenger_features`, `driver_features`): логин, REST-жизненный цикл заказа/смены, главные экраны с **картой 2GIS**, **websocket** для пассажира (`/passenger`) и водителя (`/driver`), маркер водителя на карте пассажира из realtime, офлайн-очередь + `SyncService` в bootstrap/репозиториях, навигация на историю/профиль/трекинг (пассажир) и заработок/профиль (водитель).

Ограничения мобилки по смыслу «production»: нужен собственный **бинарный** `dgissdk.key`; релиз iOS надо собирать на macOS со `pod install` (см. `mobile/flutter_taxi_app/README.md`); автопуши/crash/SDK-полировка описаны отдельно в продуктовом roadmap, а не считаются закрытыми этим репозиторием.
...
3. Мобильный контур: `flutter analyze`/CI для `flutter_taxi_app`, интеграционные сценарии (карта + WS при поднятом backend), подпись/stage-релизы, при необходимости FCM/APNs и мониторинг сбоев.
